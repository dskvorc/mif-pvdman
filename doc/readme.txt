...
- test applications in client_api/tests demonstratet how to:
  * retrieve all PvDs from MIF-PvD (pvd_list.c)
  * retrieve specific PvDs identified by (partial) id (pvd_get_by_id.c)
  * select specific PvD and run given command in it (pvd_run.c)
  * retrieve specific PvDs identified by propertied (pvd_get_by_properties.c)
...
More information for extension for PvD properties are in conf/httpd directory
...
Other tests (including properties; see conf/httpd/pvd-info-* content):
